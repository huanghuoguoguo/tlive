...
- busy 时 follow-up 的 steer / queue 语义
- 队列深度上限与 IM 内反馈
- stale 检测
- idle 超时后的自动重置
- resume 失败 fresh fallback
- `/queue`、`/diagnose`、`/home` 的状态可见性增强

当前仍未完全收口的部分：

- 会话状态机仍然隐含在多处代码中，没有独立成统一规则层
- reply-to-message、切 session、切工作区、切项目时的治理语义还需要进一步统一
- 队列策略目前偏“最小可用”，还不是最终形态

## 下一阶段目标

下一阶段的重点不是继续加命令，而是把现有治理逻辑变得一致：

1. 统一 busy / queue / reply / switch 的规则
2. 统一切工作区、切项目、`/new`、自动 reset 时的 session cleanup 语义
3. 继续增强用户可感知反馈，但不把系统复杂度转嫁给用户

一句话说，就是把当前的“会话路由器”继续推向“会话治理器”，但仍然保持 SDK 路线和 IM-first 交互。
