...
- `/cd` 无参数显示当前目录和最近历史
- 增强 `/pwd`
- workspace binding 持久化与展示

当前仍未完全收口的部分：

- “当前目录”和“长期工作区归属”的语义还需要进一步统一
- 同仓库子目录切换与跨仓库切换的 session 处理规则仍在继续细化
- 工作区与项目层的关系还没有完全收口

## 下一阶段目标

下一阶段建议继续坚持轻量治理，而不是扩出新平台抽象：

1. 让同仓库切换尽量不打断当前工作流
2. 让跨仓库切换必须显式重置上下文
3. 继续把 workspace binding 作为 repo 归属语义，而不是引入复杂 workspace 池

这里的关键目标不是“功能更多”，而是让用户在 IM 里切目录、切仓库时不再有惊讶感。
