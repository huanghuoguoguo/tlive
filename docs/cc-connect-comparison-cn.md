...
下面是按“可借鉴的 4 个方向”对当前 `tlive` 代码状态的简要盘点：

1. 会话治理（约 65%）
   已实现：
   - busy 时 steer/queue 与队列上限反馈
   - stale 检测、idle 超时重置、resume 失败 fresh fallback
   - `/queue`、`/diagnose`、`/home` 的状态可见性增强
   未完成：
   - 更完整的会话状态机和更细粒度队列策略
   - 进一步统一 reply-to-message、切会话、切工作区时的治理语义

2. 工作区治理（约 65%）
   已实现：
   - 目录历史持久化
   - `/cd -`、增强 `/pwd`、workspace binding 展示
   未完成：
   - “当前目录”和“长期工作区归属”的语义进一步收口
   - 同仓库切换与跨仓库切换的会话处理策略继续细化

3. 自动化入口（约 60%）
   已实现：
   - token 保护 webhook
   - `sessionId` / `channelType+chatId` / `projectName` 路由
   - `reject/create` 会话策略与 IM 反馈
   - 最小 cron 调度器（prompt-only）
   未完成：
   - webhook 基础频率限制
   - cron 时区/并发治理与更强观测能力
...
结论：第一轮“借鉴”已形成代码骨架，下一阶段重点不是继续扩面，而是把上述 4 条治理链路做稳、做一致。
