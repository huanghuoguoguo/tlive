...
export class TopicResumeService {
  ...
  async resume(token: string): Promise<boolean> {
    ...
    const target = this.findReportedTarget(parsed);
  ...
  private findReportedTarget(parsed: ParsedResumeToken): AgentSessionTarget | undefined {
    for (const client of this.ctx.services.getExecutionClients?.() ?? []) {
      for (const session of client.sessions ?? []) {
        if (session.sdkSessionId !== parsed.sdkSessionId) continue;
  ...
  private async bindExistingTopic(
    ...
    await this.ctx.services.router.rebind(
      ...
      existing?.sessionId ?? generateSessionId(),
      {
        ...
        clientId: target.clientId ?? existing?.clientId ?? source?.clientId ?? record.clientId,
        cwd: target.cwd,
  ...
  private async notifyExistingTopic(
    ...
    this.ctx.services.topicSessions?.upsert({
      ...
      sdkSessionId: target.sdkSessionId,
      provider: target.provider,
      clientId: target.clientId ?? record.clientId,
      cwd: target.cwd,
  ...
  private async openNewTopic(target: AgentSessionTarget): Promise<void> {
    ...
    await this.ctx.services.router.rebind(this.ctx.msg.channelType, scopeId, generateSessionId(), {
      sdkSessionId: target.sdkSessionId,
      provider: target.provider,
      clientId: target.clientId ?? currentBinding?.clientId,
      cwd: target.cwd,
