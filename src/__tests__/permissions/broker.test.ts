...
describe('PermissionBroker', () => {
  ...
  it('forwards permission request to adapter with buttons and web link', async () => {
    ...
    expect(payload).toContain('perm:allow:perm1');
    expect(payload).toContain('perm:allow_session:perm1');
    expect(payload).toContain('perm:deny:perm1');
    expect(payload).toContain('https://termlive.example.com');
  });
