...
GITHUB_REPO="termlive/termlive"
VERSION=$(curl -sf https://api.github.com/repos/${GITHUB_REPO}/releases/latest | grep -o '"tag_name": "[^"]*"' | cut -d'"' -f4 || echo "latest")

BINARY_URL="https://github.com/${GITHUB_REPO}/releases/download/${VERSION}/tlive-core-${OS}-${ARCH}"
DEST="$TERMLIVE_HOME/bin/tlive-core"

echo "Downloading tlive-core ${VERSION}..."
...
if ! command -v node &>/dev/null; then
  ...
  echo "Install from https://nodejs.org/ then run: npm install -g termlive"
  echo ""
...
echo ""
echo "Next steps:"
echo "  1. Run 'npx termlive setup' to configure IM platforms"
echo "  2. Or in Claude Code: /termlive setup"
echo ""
